...
module Parallel.Skel.DivConRW (divConRW,divConDM,divConRW2,divConDM_N,divConDM_N_NoRes) where
...
-- workpools by MD
import Parallel.Skel.EdenMWToken (mwNest')

-- edi workpool, sorting
import Parallel.EdiWP
...
divConRW2 depth trivial solve split combine x 
  ...
  where (tasks,levels) = generateTasks depth trivial split x
        results        = workpool (divConSeq trivial solve split combine) tasks
			 --------
workpool f tasks =  mwNest 1 4 noPe 2 f tasks
