...
zerlegebei xs n = splitAt (fromIntegral n) xs
...
mal  :: Poly -- ^ erster Faktor
   -> Poly -- ^ zweiter Faktor
   -> Poly -- ^ Produkt
mal1 a b = let na = length a
	       nb = length b
               ma = length $ filter (/= 0) a
               mb = length $ filter (/= 0) b
           in trace ((show ma) ++ "/" ++ (show na) ++ ", " 
                   ++ (show mb) ++ "/" ++ (show nb))
	      $ mal a b
malI'  :: Poly -- ^ erster Faktor
       -> Poly -- ^ zweiter Faktor
       -> Poly -- ^ Produkt
malI' [] b = []
malI' a [] = []
malI' (a:as) (b:bs) = (a*b):(plus (plus (malI' [a] bs) (malI' [b] as)) (0:(malI' as bs)))

malI [] b = []
malI a [] = []
-- malI [0] _ = []
-- malI _ [0] = []
-- simulate broken behaviour:
-- malI [x] ys = malI' [x] ys
-- malI xs [y] = malI' xs [y]
-- numeric prelude:
malI xs ys = foldr (\y zs -> let (v:vs) = scale y xs in v : plus vs zs) [] ys

mal' x y = simpl $ malI' (simpl x) (simpl y)
mal x y = simpl $ malI (simpl x) (simpl y)

-- ooops, wrong order!
--simpl xs = dropWhile (==0) xs
simpl xs = reverse $ dropWhile (==0) $ reverse xs

-- simpl = id

testMul = \(xs, ys) -> (mal xs ys) == (mal' xs ys)

scale :: Integer -> Poly -> Poly
scale f p = map (f *) p
...
polypot polynom n | n>0 = mal polynom (polypot polynom (n-1))  
                  | otherwise = []
polypot' p 0 = [1]
polypot' p 1 = p
polypot' p n | n<0 = []
             | even n = mal rest rest
             | otherwise = mal p $ polypot' p (n-1)
             where rest = polypot' p (n`div`2)

testPolypot = \(xs, x) -> (polypot xs x) == (polypot' xs x)
...
skalmul n xs = scale n xs

testSkal = \(x, xs) -> (skalmul x xs) == (skalmul' x xs)
...
iszeropoly xs = all (==0) xs

testZero = \xs -> (iszeropoly xs) == (iszeropoly' xs)
...
polymodulo xs m = map (\x -> x `mod` m) xs 
