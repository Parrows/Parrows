...
jacobisumteststep3seq2 xs n t lps = decide $ 
                                map worker $
                                xs
    where worker :: (Integer, Integer) -> Maybe [Integer]
          worker (p, q) = jacobisumteststep4 p k q n lps
              where k = nu (q-1) p  -- Vielfachheit von p in (q-1)
          decide [x] = x
          decide (x:xs) | x==Nothing = Nothing
                        | otherwise = decide xs
          decide _ = error "Wrong case in decide function!"
\end{code}
...
\begin{code}
-- the parallel!
jacobisumteststep3 xs n t lps | length res > 0 = last res
                              | otherwise = trace (show (xs, n, t, lps) )
                                            Nothing
    where worker (p, q) = r2
              where k = nu (q-1) p  -- Vielfachheit von p in (q-1)
                    r1 = jacobisumteststep4 p k q n lps
                    r2 | r1 == Nothing = Nothing
                       | otherwise = Just r1 -- returns Just Just x
          res = farmUntilMaybe worker xs

worker' (p, q) n t lps = r2
    where k = nu (q-1) p  -- Vielfachheit von p in (q-1)
          r1 = jacobisumteststep4 p k q n lps
          r2 | r1 == Nothing = Nothing
             | otherwise = Just r1 -- returns Just Just x
