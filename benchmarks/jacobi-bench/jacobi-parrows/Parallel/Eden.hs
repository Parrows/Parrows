...
#ifndef __SIMUL__
import qualified Parallel.ParPrim
...
noPe = unsafePerformIO Parallel.ParPrim.noPe
selfPe = unsafePerformIO Parallel.ParPrim.selfPe
#else
noPe = 1
selfPe = 1
#endif
...
class NFData a => Trans a where
    ...
    createComm = do (cx,x) <- createC
		    cx `seq` return (Comm (sendVia cx) , x)
