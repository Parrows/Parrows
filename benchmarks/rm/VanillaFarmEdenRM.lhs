...
listRabinMillerP2 :: Integer        -- ^ n ist der Primzahlkandidat
                  ...
                  -> Maybe Integer  -- ^ Die Ausgabe (Nothing: keine Primzahl, Just n: Moegliche Primzahl) 
listRabinMillerP2 n xs k = iterUntil map_farm (n, k, xs) createTask worker doneContinue
createTask (n, k, xs) | length xs > k = error "More bases than tasks"
                      | length xs < k = error "More tasks than bases"
                      | otherwise = trace (show res) res
                      where res = [(n, x) | x<-xs ]
worker :: (Integer, Integer) -> Bool
worker (n, x) = singleRabinMillerBool n t 0 b
...
doneContinue (n, k, xs) bs | bs == [] = trace "empty bs" Right []
                           | length bs /= k = trace "bs too short" Right []
                           | not $ and bs = Left Nothing
                           | and bs = Left (Just n)
                           | otherwise = error "The world cracked up"
\end{code}
\begin{badcode}
-- works, but makes only two iterations on 8 pe instead of three
listRabinMillerP2 n xs k = iterUntil map_par (n, k, xs) createTask worker doneContinue
