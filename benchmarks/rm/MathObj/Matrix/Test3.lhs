...
main = do
  ...
  putStrLn $ unlines $ ["beatSolve, test3. A parallel Gauss elimination in Eden",
                        ...
                        " 3 - parMap residue              <-- streaming",
                        " 4 - workpoolD residue BROKEN",
                        " 5 - farm residue",
                        " 6 - DM residue",
                        " 7 - workpool residue BROKEN",
                        " 9 - parMap, whole matrix        <-- chunking (both)",
                        "10 - seq. map, diagonal only     <-- chunking (both)",
                        "11 - parMap, diagonal only       <-- chunking (both)",
                        "12 - parMap, diagonal only       <-- chunking (once)",
                        "13 - parMap, diagonal only, rnf  <-- chunking (both)",
...
\begin{code}
  ...
  let res :: Fraction Integer      
      res = case t of
              2 -> detResidueG map'' ps m
              3 -> detResidueG map_par' ps m
              4 -> detResidueG (workpoolD'' nope 3) ps m
              5 -> detResidueG map_farm'' ps m
              6 -> detResidueG map_dm'' ps m
              7 -> detResidueG map_wp''' ps m
              9 -> putStrLn "----> Will send back the whole matrix!!!" `seq`
                   detResidueGC c map_par' ps m
              10 -> putStrLn "Will send back only the diagonale!" `seq`
                    putStrLn "----> using seq. map!!!" `seq`
                    detResidueGCD c map'' ps m
              11 -> putStrLn "Will send back only the diagonale!" `seq`
                    detResidueGCD c map_par' ps m
              12 -> putStrLn "Will send back only the diagonale!" `seq`
                    detResidueGCL c map_par' ps m
              13 -> putStrLn "Will send back only the diagonale!" `seq`
                    detResidueGCD2 c map_par' ps m

              _ -> error "wrong type"
  c1 <- getClockTime
  print res -- force the evaluation
  c2 <- getClockTime
  putStrLn $ "Total Runtime: @ " 
               ++ (showT $ diffClockTimes c2 c1) ++ " @"
  putStrLn "done"
...
showT t = let m = fromIntegral $ tdMin t * 60
              s = fromIntegral $ tdSec t
              ps :: Double
              ps = (fromIntegral $ tdPicosec t) / 10^12
          in (show $ m + s + ps) ++ " seconds"
