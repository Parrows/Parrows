...
convToInt :: Mod Integer -> Mod Int
convToInt (Z x y) = Z (fromInteger' x) (fromInteger' y)

convToInteger :: Mod Int -> Mod Integer
convToInteger (Z x y) = Z (toInteger' x) (toInteger' y)

-- makeZinteger :: Integer -> Integer -> Mod Integer
-- makeZinteger = makeZ

makeIZ'' :: Integer -> [Int] -> IMods Int
makeIZ'' x ps = 
    let ps' = map toInteger' ps
        r' = map (makeZ x) ps'
    in map convToInt r'
\end{code}
...
restoreIZinteger :: IMods Int -> Mod Integer
restoreIZinteger input 
    = convertMixedRadix $ mixedRadix $ map convToInteger input
...
instance (Integral a) => Num (IMods a) where
    ...
    (-) = lift2 (-)
    (*) = lift2 (*)
    fromInteger = error "fromInteger in IntMulti module: no residue class specified!"
