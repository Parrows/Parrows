...
import Data.Maybe
-- import Data.Either -- not compatible with vectorizer
\end{code}
\begin{code}
...
listRabinMillerP2 :: Integer        -- ^ n ist der Primzahlkandidat
                  ...
                  -> Maybe Integer  -- ^ Die Ausgabe (Nothing: keine Primzahl, Just n: Moegliche Primzahl) 
listRabinMillerP2 n xs k = iterUntil k map (n, k, xs) createTask worker doneContinue
...
doneContinue :: (Integer, Int, [Integer]) -> 
                ...
                Either (Maybe Integer) [(Int, Integer, Integer)]
-}
doneContinue (n, _, xs) bks | not $ and bs = makeLeft Nothing
                            | k<=0 = makeLeft (Just n)
                            | otherwise = trace ("Iterating for another " ++ (show $ k-length bs))
                                          $ makeRight (createTask (n, k-length bs, xs))
