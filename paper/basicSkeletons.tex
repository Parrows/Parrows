...
\\\\
We start by transforming the \code{(a, c)} input into a 2-element list \code{[Either a c]}. Then, we feed this list into a parallel arrow constructed with \code{parEvalN} that uses \code{f +++ g}  we then feed into a 2-element parallel arrow that we created as described above. After the calculation is finished we convert the resulting \code{[Either b d]} into \code{([b] , [d])}. The two lists in this tuple contain only 1 element each by construction, so we can finally just convert the tuple to \code{(b, d)} in the last step.
\begin{lstlisting}[frame=htrbl]
