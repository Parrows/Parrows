...
\\
The mapArr combinator, that lifts any arrow \code{arr a b} to an arrow \code{arr [a] [b]} \cite{programming_with_arrows},
...
\end{lstlisting}
and zipWithArr, which lifts any arrow \code{arr (a, b) c} to an arrow \code{arr ([a], [b]) [c]}.
\begin{lstlisting}[frame=htrbl]
