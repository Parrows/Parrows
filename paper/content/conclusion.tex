...
computation. Instead, we use Arrows in the same manner one uses sequential pure functions. Even though we make heavy use of the |ArrowApply| typeclass - which only Arrows that are equivalent to Monads can satisfy - in this paper, we believe that the general idea of an Arrow, the process of doing something - a composable \emph{computation}, is the perfect fit to use in a library for parallel computation as it allows users to think about parallel programs more as a set of composed computations - like a data-flow graph. Furthermore, our approach allows for a more natural paralellization of any Arrow type (especially regular functions (|->|)), when compared to Monads as we do not have to change anything in order to run them in parallel.
%
