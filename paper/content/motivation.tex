...
the parallel backends at will. The induced penalty was max. 8.3 \% in our measurements \mbcomment{, and typically under X \%}. Further backends based on HdpH or a Frege implementation (on the Java Virtual Machine) are viable, too.
...
We propose an Arrow-based encoding for parallelism based on a new Arrow combinator |parEvalN :: [arr a b] -> arr [a] [b]| in Section \ref{sec:parallel-arrows}, which converts a list of Arrows into a new parallel Arrow. A parallel Arrow is still an Arrow, hence the resulting parallel Arrow can still be used in the same way as a potential sequential version.

In this paper evaluate the expressive power of our Arrow formalism in the context of parallel programming.

\begin{itemize}
\item We introduce a parallel evaluation formalism using Arrows. One big advantage is that we do not have to introduce any new types (Sec.~\ref{sec:parallel-arrows}) in contrast to Monad solutions such as the |Par| Monad. This behaviour encourages better composability.
