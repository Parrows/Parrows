...
\ensuremath{(\mathbin{*\!*\!*})}\label{fig:***Img} & \ensuremath{(\mathbin{\&\!\&\!\&})} \label{fig:&&&Img} \\
	\end{tabular}
	\caption{Visual depiction of derived combinators for Arrows.}
	\label{fig:syntacticSugarArrows}
\end{figure}
Hughes also derived some further combinators (Fig.~\ref{fig:syntacticSugarArrows}): \ensuremath{\Varid{second}}, \ensuremath{\mathbin{*\!*\!*}} and \ensuremath{\mathbin{\&\!\&\!\&}}. 
...
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{parEvalN}\;\Varid{fs}\;\Varid{as}\mathrel{=}\Varid{runPar}\mathbin{\$}\Varid{mapM}\;(\Varid{return}\mathbin{>>>}\Varid{spawn}\bind \Varid{get})\mathbin{\$}\Varid{zipWith}\;(\mathbin{\$})\;\Varid{fs}\;\Varid{as}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
Here, the \ensuremath{\Conid{Trans}} typeclass originates from the Eden standard library and details the implementation of packing and of the transmission. A~default instance of \ensuremath{\Conid{Trans}} exists.
...
\end{hscode}\resethooks
 %$ %% formatting
\caption{Definition of \ensuremath{\Varid{parEvalNLazy}}. The type \ensuremath{\Conid{ChunkSize}} is an alias to \ensuremath{\Conid{Int}}.}
...
If we used the original definition of \ensuremath{\Varid{parEvalN}}, however, these skeletons would produce an infinite loop with the GpH and \ensuremath{\Conid{Par}} Monad which during runtime would result in program crash. This materialises with the usage of \ensuremath{\Varid{loop}} of the \ensuremath{\Conid{ArrowLoop}} type class and we think that this is due to difference of how evaluation is done in these backends compared to Eden. We provide a workaround for these types of skeletons as such they probably are not of much importance outside of a distributed memory environment. However our workaround enables users of the DSL to test their code within a shared memory setting.
...
but the definition was then unified with \ensuremath{\Varid{evalN}}. 
...
\section{Derived combinators} \label{syntacticSugar}
Finally, we also give the definitions for some derived combinators for PArrows, namely \ensuremath{\mathbin{\mid\!\!*\!*\!*\!\!\mid}} and \ensuremath{\mathbin{\mid\!\!\&\!\&\!\&\!\!\mid}}.
