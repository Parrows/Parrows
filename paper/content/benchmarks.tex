...
\caption{Relative speedup of \rmtest on a multicore machine. We used the same PArrows-based implementation with
  different backends on the same hardware. Measurements were performed on a single node of Glasgow
...
efficiency was 85.3\%.  However, when we increase the number of
requested cores to be 32---\ie when we use hyperthreading on 16 real
...
In the distributed memory setting same effect ensues. We obtain
...
versions. These are the values provided by |Criterion| package that we
