...
The program does not seem to scale well beyond 64 cores with input $2^{3217}-1$.

Because of this, we also ran tests with input $2^{4253}-1$, but only for the 128 and 256 cores (and therefore not pictured in the speedup figure) because of the long running time. For 128 threads, the benchmark took a mean of 9192.9\seconds, while with 256 threads, it only took a mean of 1649.1\seconds. This means that the 128 thread version ran more than 5.5 times slower than the 256 one, which suggests an IO limitation for this big input that is somewhat mitigated by adding more cores. However it shows that PArrows continue to scale, even if not perfectly for this test program.
