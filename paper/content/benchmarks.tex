...
\newcommand{\sudokutest}{Sudoku solver\xspace}
\newcommand{\jacobitest}{Jacobi sum test\xsp}
...
As the last Benchmark in this paper we present the \sudokutest in \ref{fig:sudokuSMBenchmark} running in a shared memory setting. Here we see all three SM backends performing similar again like in the \rmtest SM benchmarks. \mbcomment{Benchmarks seem a bit off. redoing atm}
...
\speedupplot{ \sudokutest ``#1''}{Eden CP, Multicore, |Par| Monad}{16}{4}{
\addplot+ [very thick] table [scatter, x="nCores", y="speedup", col sep=comma, mark=none,
smooth]{benchmarks/sudoku-sm/bench-sudoku-sm.bench.parrows-sudoku-parmap-eden-sudoku17.#1.txt.csv};
\addplot+ [very thick] table [scatter, x="nCores", y="speedup", col sep=comma, mark=none,
smooth]{benchmarks/sudoku-sm/bench-sudoku-sm.bench.parrows-sudoku-parmap-mult-sudoku17.#1.txt.csv};
