...
The |Par| monad and GpH Haskell -- in its multicore version \cite{Marlow2009} --  can be executed on a shared
...
The program does not seem to scale well beyond 64 cores with input $2^{3217}-1$. Because of this, we also ran tests with input $2^{4253}-1$, but only for the 128 and 256 cores because of the long running time. For demonstration purposes in Fig.~\ref{fig:bench-jacobi-dist}, we chose a \enquote{simulated} speedup of 22 for 128 cores. This was obtained by taking the speedup value of 64 cores (approx. 11) of the $2^{3217}-1$ and assuming optimal \enquote{linear} speedup between 64 and 128 cores. For 256 cores, this results in a superlinear \enquote{speedup} of \~122.63 relative to 128 cores, which suggests an IO limitation for this big input that is somewhat mitigated by adding more cores.
...
Next is the \torustest benchmark. The results of the comparison of vanilla Eden to our PArrows-based version can be found in Fig.~\ref{fig:torusBenchmark}. We see that the benchmark scales quite well with more cores until 64 cores. For more cores (>=96) we still have considerable speedup, but it increases slower. We also prove that the difference between the Eden and PArrows version are only again marginal with PArrows only being a maximum of $1.7\%$ slower -- for 160 cores -- when outside of the error bar.  %The difference between PArrows and Eden is only significant for 16 and 64 cores where it ran 1.7\% and 2.7\% slower which corresponds to a real-time difference of 0.12s and 0.13s. For 256 cores PArrows performed 0.2\% slower which corresponds to 0.01s overhead.
...
\performanceplot{Parallel run time of \sudokutest \enquote{#1}}{linear speedup, Eden CP, GpH, |Par| Monad}{16}{4}{
