...
Note that with the Kleisli type we can define an alternative to |(>>=)| with the signature |m a -> Kleisli m a b -> m b|. The difference between the definitions of Arrows and Monads becomes more clear if we compare this with the signature of the Arrow compose operator |(>>>) :: arr a b -> arr b c -> arr a c|: Monads are types that allow composition of computations, while Arrows \emph{are} composable computations. 
