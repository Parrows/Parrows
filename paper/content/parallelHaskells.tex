...
For |parEvalN| this means that we can just apply the list of functions |[a -> b]| to the list of inputs |[a]| by zipping them with the application operator |$|. % $
...
The resulting code and a graphical representation can be found in Fig.~\ref{fig:parEvalNMulticore} and Fig.~\ref{fig:parEvalNMulticoreImg}, respectively.
...
The |Par| monad\footnote{It can be found in the \texttt{monad-par} package on hackage under \url{https://hackage.haskell.org/package/monad-par-0.3.4.8/}.} introduced by \citet{par-monad}, is a monad designed for composition of parallel programs.


The |Par| Monad version of our parallel evaluation function |parEvalN| can be defined by zipping the list of |[a -> b]| with the list of inputs |[a]| with the application operator |$| just like with Multicore Haskell. % $
...
Again, the resulting code and a graphical representation can be found in Fig.~\ref{fig:parEvalNParMonad} and Fig.~\ref{fig:parEvalNParMonadImg}, respectively.
