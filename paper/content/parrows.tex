...
The Multicore Haskell implementation of |ArrowParallel| is implemented in a straightforward manner by using |listApp| (Appendix~\ref{utilfns}) combined with the |withStrategy :: Strategy a -> a -> a| combinators from Multicore Haskell, where |withStrategy| is the same as |using :: a -> Strategy a -> a|, but with flipped parameters.
\begin{figure}[t]
\begin{code}
