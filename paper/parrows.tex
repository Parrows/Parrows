...
As we have seen earlier, in its purest form, parallel computation (on functions) can be seen as the execution of some functions \code{a -> b} in parallel:
\begin{lstlisting}[frame=htrbl]
parEvalN :: [a -> b] -> [a] -> [b]
\end{lstlisting}
Translating this into arrow terms gives us a new operator \code{parEvalN} that lifts a list of arrows \code{[arr a b]} to a parallel arrow \code{arr [a] [b]} (This combinator is similar to our utility function \lstinline{listApp}, but does parallel instead of serial evaluation).
\begin{lstlisting}[frame=htrbl]
parEvalN :: (Arrow arr) => [arr a b] -> arr [a] [b]
\end{lstlisting}
With this definition of \code{parEvalN}, parallel execution is yet another arrow combinator. But as the implementation may differ depending on the actual type of the arrow \code{arr} and we want this to be an interface for different backends, we have to introduce the new typeclass \code{ArrowParallel} to host this combinator.
\begin{lstlisting}[frame=htrbl]
...
The Multicore Haskell implementation of this class is implemented in a straightforward manner by using listApp from chapter \ref{utilfns} combined with the \code{withStrategy :: Strategy a -> a -> a} and \code{pseq :: a -> b -> b} combinators from Multicore Haskell, where \code{withStrategy} is the same as \code{using :: a -> Strategy a -> a} but with flipped parameters.
\begin{lstlisting}[frame=htrbl]
...
For most cases this fully evaluating version would probably suffice, but as the Multicore Haskell interface allows the user to specify the level of evaluation to be done via the \code{Strategy} interface, we want to the user not to lose this ability because of using our API. We therefore introduce the \code{Conf a} data-type that simply wraps a \code{Strategy a}. We can't directly use the \code{Strategy a} type here as GHC (at least in the versions used for development in this paper) does not allow type synonyms in type class instances.
\begin{lstlisting}[frame=htrbl]
data Conf a = Conf (Strategy a)
\end{lstlisting}
We simply unwrap the strategy out of this wrapper and get the following (configurable) \code{ArrowParallel} instance:
\begin{lstlisting}[frame=htrbl]
...
\end{lstlisting}
\subsection{ParMonad}
The ParMonad implementation makes use of Haskells laziness and ParMonad's \code{spawnP :: NFData a => a -> Par (IVar a)} function. The latter forks away the computation of a value and returns an \code{IVar} containing the result in the \code{Par} monad.
...
class (Arrow arr) => ArrowUnwrap arr where
	arr a b -> (a -> b)
\end{lstlisting} %
we don't do it here, for aesthetic resons. For now, we just implement \code{ArrowParallel} for normal functions
\begin{lstlisting}[frame=htrbl]
