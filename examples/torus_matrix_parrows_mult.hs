...
myMap :: (NFData b) => (a -> b) -> [a] -> [b]
myMap fn (a:as) = let curB = fn a
                  in (rnf $ curB) `pseq` curB:(myMap fn as)
myMap fn [] = []
...
main = do
        ...
        let cMatricesExploded = parMapStream () 1 (uncurry $ matMultTorus nodeCountVal problemSizeVal) $ zipWith (,) aMatrices bMatrices
